...
export const get = query({args: {
    ...
    const allConversationMembership = await ctx.db.query("conversationMembers").withIndex("by_conversationId", q => q.eq("conversationId", args.id)).collect()
    ...
    if(!conversation.isGroup) {
        ...
        const otherMembers = (await Promise.all(allConversationMembership.filter(membership => membership.memberId !== currentUser._id)
